...
class PythonFAQChatbot(object):
    ...
    def clean_column(self, col):
        ...
        self.df['preprocessed' + col] = self.df[col].apply(
            lambda x: " ".join([word for word in x.split(" ") if word not in stop_words]))
        self.df['preprocessed' + col] = self.df['preprocessed' + col].str.replace('[^a-zA-Z0-9 ]', '')
        self.df['preprocessed' + col] = self.df['preprocessed' + col].str.lower()
    ...
    def create_clean_text_column(self, column):
        ...
        # self.df['clean_title'] = self.df['Title'].swifter.apply(self.clean_column)
        # self.df['clean_body'] = self.df['Body'].swifter.apply(self.clean_column)
...
def main():
    ...
    def echo(update, context):
        ...
        answer = ml.get_answer_for_most_similar_title_plus_body(update.message.text)
