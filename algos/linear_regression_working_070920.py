...
class MlLinearRegression(MlBase):
    ...
    def plot_predicted_price_and_feature_importance(self, df, regression=None):
        print('#' * 50)
...
def do_eda(ml):
    ...
    single_valued_colmmns = ml.get_n_valued_categorical_columns(ml.df_categorical, n=1)
    ml.df_categorical.drop(columns=single_valued_colmmns, inplace=True)
...
def orginal_regression_submitted():
    ...
    ml.plot_predicted_price_and_feature_importance(df_with_dummies, regression='sm_ols')
...
def linear_regression(regression=None):
    ...
    # predict on split data
    ml.plot_predicted_price_and_feature_importance(df_with_dummies, regression=regression)
