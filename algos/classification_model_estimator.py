...
from algos.ada_boost import ada_boost
from algos.gradient_boosting import gradient_boost
from algos.knn import knn
from algos.keras_classifier import keras_classifier

from constants import TELCO_CHURN_DATA, TELCO_CHURN_DATA_TEST

# target and test data files
TRAINING_DATA_CSV_FILE = TELCO_CHURN_DATA
TEST_DATA_CSV_FILE = TELCO_CHURN_DATA_TEST
...
class MlClassificationModelEstimator(ClassificationBase):
    def __init__(self, target_variable):
        ...
        super(MlClassificationModelEstimator, self).__init__(target_variable)
...
def evaluate_models(test_data_file, target_variable):
    ...
    # Do EDA (w eshould use all features when trying out with test file?)
...
if __name__ == '__main__':
    # Run each model
    logistic_regression(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE) # with scaler and removal of high correlated features
    decision_tree(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    random_forest(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    xg_boost(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    ada_boost(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    mlp(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    state_vector_machine(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    knn(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    gradient_boost(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
    # keras_classifier(data_file=TRAINING_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)

    # Now Evaluate all models using model (pkl) files generated for each model above
    evaluate_models(test_data_file=TEST_DATA_CSV_FILE, target_variable=TARGET_VARIABLE)
