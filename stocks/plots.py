...
class TechnicalIndicatorsChartPlotter():
    ...
    def plot_price_and_signals(self, fig, data, strategy, axs):
        ...
        fig.suptitle(f'Top: {self.symbol} Stock Price. Bottom: {strategy}')

        if not data[f'{strategy}_Buy'].isnull().all():
            axs[0].scatter(data.index, data[f'{strategy}_Buy'], color='green',
                           label='Buy Signal', marker='^', alpha=1)
        if not data[f'{strategy}_Sell'].isnull().all():
            axs[0].scatter(data.index, data[f'{strategy}_Sell'], color='red',
                           label='Sell Signal', marker='v', alpha=1)
        axs[0].plot(self.prices, label='Close Price', color='blue', alpha=0.35)
    ...
    def plot_macd(self):
        image = f'images/{self.symbol}_macd.png'
        macd = self.technical_indicators

        # Create and plot the graph
        fig, axs = plt.subplots(2, sharex=True, figsize=(13, 9))
        self.plot_price_and_signals(fig, macd, 'MACD', axs)

        axs[1].plot(macd['MACD'], label=self.symbol + ' MACD', color='green')
    ...
    def plot_rsi(self):
        image = f'images/{self.symbol}_rsi.png'
        rsi = self.technical_indicators
        low_rsi = 40
        high_rsi = 70

        # plt.style.use('default')
        fig, axs = plt.subplots(2, sharex=True, figsize=(13, 9))
        self.plot_price_and_signals(fig, rsi, 'RSI', axs)
    ...
    def plot_bollinger_bands(self):
        image = f'images/{self.symbol}_bb.png'
        bollinger_bands = self.technical_indicators

        fig, axs = plt.subplots(2, sharex=True, figsize=(13, 9))

        self.plot_price_and_signals(fig, bollinger_bands, 'Bollinger_Bands', axs)

        axs[1].plot(bollinger_bands['Bollinger_Bands_Middle'], label='Middle', color='blue',
                    alpha=0.35)
        axs[1].plot(bollinger_bands['Bollinger_Bands_Upper'], label='Upper', color='green',
                    alpha=0.35)
        axs[1].plot(bollinger_bands['Bollinger_Bands_Lower'], label='Lower', color='red',
                    alpha=0.35)
